        ...
        <div className="flex items-center justify-between h-12">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map(({ href, label }) => {
              const isLightLink = !isScrolled && (label === 'OUR STORY' || label === 'INSIGHTS');
              return (
                <Link
                  key={href}
                  href={href}
          ...
          <div className="hidden md:flex">
            <Link 
              href="/contact" 
              className={cn(
                "flex items-center uppercase space-x-2 font-semibold underline text-sm transition-colors duration-200",
                isScrolled 
